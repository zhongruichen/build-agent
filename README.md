# 多智能体助手 (Multi-Agent Helper) v2.3

**一个具备规划、执行、反思、自我修正和可视化工作流构建能力的VS Code多智能体开发框架。**
...
```bash
code --install-extension multi-agent-helper-2.3.0.vsix
```
...
### 🎨 可视化工作流构建 (NEW!)
现在您可以通过直观的拖拽式界面创建复杂的自动化工作流！这个全新的可视化工作流编辑器让您能够：

- **拖拽式节点创建**: 从节点库拖拽创建工作流节点，支持9种节点类型
- **可视化连接**: 通过简单的点击和拖拽连接节点，构建执行流程
- **实时属性编辑**: 在属性面板中即时配置节点参数
- **YAML双向转换**: 在可视化界面和YAML代码之间无缝切换
- **预置模板**: 5个开箱即用的工作流模板（文件处理、Git操作、代码分析、测试运行、部署流程）
...
- 📝 **顺序节点**: 按顺序执行任务
- ❓ **条件节点**: 基于条件的分支执行
- 🔄 **循环节点**: 重复执行任务（forEach/while/count）
- ⚠️ **错误处理**: try-catch-finally 结构
- 🔀 **数据转换**: map/filter/reduce 操作
- 🔗 **合并节点**: 合并多个输入
- ✂️ **分割节点**: 分割数据流

#### 如何使用工作流编辑器
1. 按下 `Ctrl/Cmd + Shift + P`，选择 **"工作流: 打开工作流编辑器"**
2. 从左侧节点库拖拽节点到画布
3. 切换到连接模式（🔗）创建节点间的连接
4. 在右侧属性面板配置节点参数
5. 点击"验证"确保工作流正确
...
多智能体助手现已集成 **MCP 协议**，这是一个标准化的协议，用于连接 LLM 应用程序和外部工具。通过 MCP，您可以：

- **连接外部服务**: 轻松集成任何 MCP 兼容的服务器，如文件系统、数据库、API等
- **暴露内部工具**: 将插件的所有工具通过 MCP 协议暴露，供其他应用使用
- **无限扩展性**: 通过标准化协议，智能体可以访问更广泛的工具和资源

详细的 MCP 配置和使用说明请参阅 **[MCP 集成指南](MCP_GUIDE.md)**。
...
- **MCP 协议支持**: 通过标准化协议连接外部工具和服务，无限扩展智能体能力。
...
- **MCP 集成?** 查看 [**MCP_GUIDE.md (MCP 集成指南)**](MCP_GUIDE.md)
