...
class ToolOrchestrationEngine extends EventEmitter {
    ...
    async executeWorkflow(workflow, context = {}) {
        ...
        try {
            ...
            // Execute stages
            const result = await this.executeStages(workflowDef.stages, execution);
            // Complete execution
    ...
    async executeParallel(stages, execution) {
        const promises = stages.map(stage => this.executeStage(stage, execution).catch(error => ({
            error: error instanceof Error ? error.message : String(error),
            stage
        })));
    ...
    async executeConditional(conditional, execution) {
        ...
        if (condition) {
            if (conditional.then) {
                return await this.executeStages(Array.isArray(conditional.then) ? conditional.then : [conditional.then], execution);
            }
        }
        else {
            if (conditional.else) {
                return await this.executeStages(Array.isArray(conditional.else) ? conditional.else : [conditional.else], execution);
            }
        }
        return null;
    }
    ...
    * @private
    ...
    async executeTransform(transform, execution) {
        ...
        if (transform.type === 'reduce') {
            ...
            return input.reduce((/** @type {any} */ acc, /** @type {any} */ item) => this.applyExpression(transform.expression, { acc, item }), transform.initial || null);
        }
    ...
    async executeMerge(merge, execution) {
        const inputs = merge.inputs.map((/** @type {any} */ input) => this.resolveValue(input, execution.context));
    ...
    async rollbackExecution(execution) {
        ...
        while (execution.rollbackStack.length > 0) {
            ...
            try {
                await this.executeToolHandler(this.toolRegistry.get(rollback.tool), this.resolveParams(rollback.params, execution.context));
...
//# sourceMappingURL=ToolOrchestrationEngine.js.map