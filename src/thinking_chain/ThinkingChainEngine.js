...
class ThinkingChainEngine {
    ...
    buildDynamicPrompt(thinkingConfig = {}) {
        ...
        if (thinkingConfig.depth) activeDirectives.push(`!depth(${thinkingConfig.depth})`);
        if (thinkingConfig.iterate) activeDirectives.push(`!iterate(${thinkingConfig.iterate})`);
        if (thinkingConfig.model) activeDirectives.push(`!model(${thinkingConfig.model})`);
        if (thinkingConfig.focus) activeDirectives.push(`!focus(${thinkingConfig.focus})`);

        // Functional directives
        if (thinkingConfig.visualize) activeDirectives.push(`!think`);
        if (thinkingConfig.suggest) activeDirectives.push(`!suggest`);
        if (thinkingConfig.parallel) activeDirectives.push(`!parallel`);
        if (thinkingConfig.trace) activeDirectives.push(`!trace`);
        if (thinkingConfig.confidence) activeDirectives.push(`!confidence`);
        if (thinkingConfig.critique) activeDirectives.push(`!critique`);
