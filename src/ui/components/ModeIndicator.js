...
class ModeIndicator {
    ...
    createStatusBarItems() {
        ...
        // Persona indicator
        this.personaStatusBarItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            99
        );
        ...
        this.capabilityStatusBarItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            98
        );
        ...
        this.context.subscriptions.push(
            this.modeStatusBarItem,
            this.personaStatusBarItem,
            this.capabilityStatusBarItem
        );
    }
    ...
    registerCommands() {
        // Show mode menu
        this.context.subscriptions.push(
            vscode.commands.registerCommand('multiAgent.showModeMenu', async () => {
                const modes = [
                    {
        ...
        // Show persona menu
        this.context.subscriptions.push(
            vscode.commands.registerCommand('multiAgent.showPersonaMenu', async () => {
                const personas = Array.from(this.modeManager.personas.entries()).map(
                    ([name, persona]) => ({
                        label: `$(person) ${name}`,
        ...
        // Show capabilities
        this.context.subscriptions.push(
            vscode.commands.registerCommand('multiAgent.showCapabilities', () => {
        ...
        this.context.subscriptions.push(
            vscode.commands.registerCommand('multiAgent.switchToDirect', () => {
    ...
    updateIndicators() {
        ...
        if (capabilities.tools) capabilityIcons.push('$(tools)');
        if (capabilities.agents) capabilityIcons.push('$(organization)');
    ...
    async showModeSuggestion(data) {
        ...
        const action = await vscode.window.showInformationMessage(
            `${data.reason}. Switch to ${modeName} mode?`,
            'Yes',
            'No',
            'Always Auto-Switch'
        );
    ...
    async createNewPersona() {
        ...
        vscode.window.showInformationMessage(
            `Created and activated persona: ${name}`,
            { modal: false }
        );
    }
    ...
    showCapabilitiesPanel() {
        ...
        panel.webview.html = this.getCapabilitiesHtml(
            mode,
            capabilities,
            persona,
            metrics
            ...
            <body>
                ...
                <div class="section">
                    ...
                    <div class="capability">
                        ...
                        <span>Agents: ${
                            capabilities.agents === true ? 'Fully Enabled' :
                            capabilities.agents === 'selective' ? 'Selective' :
                            'Disabled'
                        }</span>
                    </div>
