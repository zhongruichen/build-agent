...
class WorkflowService {
     ...
     */
    createEditorPanel() {
        const panel = vscode.window.createWebviewPanel('workflowEditor', '工作流编辑器', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'runWorkflow':
                    try {
                        const result = await this.executeWorkflow(message.workflow.id, message.context || {});
                        panel.webview.postMessage({
    ...
    getEditorHtml(webview) {
        const scriptUri = webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'src', 'ui', 'components', 'WorkflowEditor.js')));
        const managerUri = webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, 'src', 'workflow', 'WorkflowVisualManager.js')));
        return `<!DOCTYPE html>
        <html lang="zh-CN">
        <head>
            <meta charset="UTF-8">
