...
class OrchestratorAgent extends BaseAgent {
    ...
    async executeTask(taskContext) {
        ...
        try {
            ...
            if (responseObject && Array.isArray(responseObject.plan) && responseObject.plan.every(/** @param {any} t */ t => t.id && t.description && Array.isArray(t.dependencies))) {
            ...
            if (jsonMatch && jsonMatch[1]) {
                try {
                    const parsed = JSON.parse(jsonMatch[1]);
                     if (parsed && Array.isArray(parsed.plan) && parsed.plan.every(/** @param {any} t */ t => t.id && t.description && Array.isArray(t.dependencies))) {
                        return parsed.plan;
                    }
                } catch (parseError) {
