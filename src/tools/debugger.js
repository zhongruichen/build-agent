...
async function start(configName) {
  ...
  try {
    ...
    return { success: false, error: `Failed to start debugging session: ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
async function stop() {
  ...
  try {
    ...
    return { success: false, error: `Failed to stop debugging session: ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
async function addBreakpoint(file, line) {
  try {
    ...
    return { success: false, error: `Failed to add breakpoint: ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
async function removeBreakpoint(file, line) {
    try {
        ...
        return { success: false, error: `Failed to remove breakpoint: ${error instanceof Error ? error.message : String(error)}` };
    }
}
...
async function sendDebugCommand(command) {
  ...
  try {
    ...
    return { success: false, error: `Failed to send command "${command}": ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
async function evaluate(expression) {
    ...
    try {
        ...
        return { success: false, error: `Failed to evaluate expression: ${error instanceof Error ? error.message : String(error)}` };
    }
}
