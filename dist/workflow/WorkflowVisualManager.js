...
class WorkflowVisualManager extends EventEmitter {
    ...
    removeNode(nodeId) {
        ...
        // 删除与节点相关的所有连接
        this.currentWorkflow.connections = this.currentWorkflow.connections.filter((/** @type {{ source: string; target: string; }} */ c) => c.source !== nodeId && c.target !== nodeId);
        // 删除节点
    ...
    getChildNodes(nodeId, workflow) {
        ...
        return childConnections.map(c => workflow.nodes.find(n => n.id === c.target)).filter(/** @returns {n is Node} */ /** @returns {n is Node} */ n => !!n);
    }
    /**
     * 从YAML加载工作流
    ...
    fromYAML(yamlContent) {
        try {
            ...
            const workflow = this.createWorkflow(yamlObject.name || 'Imported Workflow', yamlObject.description || '');
...
//# sourceMappingURL=WorkflowVisualManager.js.map