...
function initializeAgents(config, agentMessageBus) {
    ...
    if (!workerProfile) throw new Error("Worker role profile not found.");
    ...
    for (const roleName of requiredRoles) {
        ...
        if (!profile) throw new Error(`${roleName} role profile not found.`);
        const modelConfig = getModelForRole(roleName);
        if (!modelConfig) throw new Error(`Model for ${roleName} not found.`);
...
function detectLanguage(artifact) {
    if (!artifact || typeof artifact !== 'string') return 'plaintext';
    const trimmedArtifact = artifact.trim();
    if (trimmedArtifact.startsWith('{') || trimmedArtifact.startsWith('[')) return 'json';
    if (trimmedArtifact.startsWith('<!DOCTYPE html>') || trimmedArtifact.startsWith('<html')) return 'html';
    if (trimmedArtifact.includes('import React') || trimmedArtifact.includes('className=')) return 'javascriptreact';
    if (trimmedArtifact.includes('function') || trimmedArtifact.includes('const') || trimmedArtifact.includes('let') || trimmedArtifact.includes('=>')) return 'javascript';
    if (trimmedArtifact.includes('def ') && trimmedArtifact.includes(':')) return 'python';
    if (trimmedArtifact.includes('public class') || trimmedArtifact.includes('import java.')) return 'java';
    if (trimmedArtifact.includes('#include')) return 'cpp';
    if (trimmedArtifact.includes('<?php')) return 'php';
    return 'plaintext';
}
...
async function runIterationLoop(taskContext, agents, config, agentMessageBus, taskEventEmitter, state, signal) {
    ...
    for (let i = taskContext.currentIteration - 1; i < MAX_ITERATIONS; i++) {
        ...
        if (!evaluatorProfile) throw new Error("Evaluator role profile not found.");
...
async function runTaskExecution(taskContext, config, agentMessageBus, taskEventEmitter, state, signal) {
    ...
    if (config.get('enableLongTermMemory', false)) {
        ...
        try {
            ...
            if (!extractorProfile) throw new Error("KnowledgeExtractor role profile not found.");
            const modelConfig = getModelForRole('KnowledgeExtractor');
            if (!modelConfig) throw new Error("Model for KnowledgeExtractor not found.");
