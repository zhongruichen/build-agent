...
class WorkflowEditor {
    ...
    handleCanvasMouseUp(e) {
        ...
        if (this.connectionStart) {
            ...
            if (targetNode && targetNode !== this.connectionStart.node) {
                try {
                    this.workflowManager.connectNodes(this.connectionStart.node.id, 0, targetNode.id, 0);
    ...
    zoomToFit() {
        const workflow = this.workflowManager.currentWorkflow;
        if (!workflow || workflow.nodes.length === 0 || !this.canvas)
            return;
        // 计算边界
    ...
    updateStatusBar() {
        ...
        if (nodeCount)
            nodeCount.textContent = workflow.nodes.length.toString();
        const connectionCount = document.getElementById('connection-count');
        if (connectionCount)
            connectionCount.textContent = workflow.connections.length.toString();
    }
    /**
     * @param {string} color
     * @param {number} amount
